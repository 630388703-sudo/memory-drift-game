...
LOOT 当前版本将三轨走廊压缩为约 30 秒的记忆追逐：顶部持续提示下一扇门位于左、中或右轨，玩家通过三扇记忆门并沿途收集五枚线索微光；声音与照片碎片会在途中直接写入档案，第三扇门后立即进入入口选择，不再无目的回环。Version A/B 也改为两张不同色调、人物轮廓和照片背面批注的档案卡。版本同时保留原创竖屏场景、胶片扫描线、雨幕、漂尘、视差、锚点共振、循环教室校准、九宫格碎片拼合和双版本结局。每个阶段首次进入自动显示操作教学，H 可再次查看；L 切换中英文，F 全屏，M 静音。结局导出的 JSON 只包含本次游戏行为，不记录姓名、账号、联系方式、位置或设备身份。
